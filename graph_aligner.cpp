...
//Source constructor
GraphAligner::Node::Node(Kmer kmer, double prob)
    : length_(1),
      stay_count_(0),
      skip_count_(0),
      ignore_count_(0),
...
//Child constructor
GraphAligner::Node::Node(Node *parent, Kmer kmer, 
...
//Copy constructor
GraphAligner::Node::Node(const Node &s)
    : length_(s.length_),
...
bool GraphAligner::Node::is_valid() {
    return length_ > 0;
}

size_t GraphAligner::Node::seed_len() {
    return length_;
}

size_t GraphAligner::Node::match_len() {
    return seed_len() - stay_count_ - ignore_count_ + skip_count_;
}

//TODO: Really not a very good name
bool GraphAligner::Node::better_than(const Node *node) {
    return node->ignore_count_ > ignore_count_ ||
            (node->ignore_count_ == ignore_count_ && 
             node->seed_prob_ < seed_prob_);
}

bool GraphAligner::Node::should_report(const AlnParams &params) {
...
void GraphAligner::Node::invalidate(std::vector<Node *> *old_nodes, 
...
void GraphAligner::Node::replace_info(const Node &node) {
...
void GraphAligner::Node::update_info() {
    if (parents_.empty()) {
...
bool GraphAligner::Node::remove_child(Node *child) {
...
void GraphAligner::Node::print() const {
    //std::cout << length_ << "\n";
              //<< full_length_ << "\n";
              //<< stay_count_ << "\t"
              ////<< skip_count_ << "\t"
...
std::vector<Result> GraphAligner::add_event(Event e, std::ostream &out) {
    ...
    for (auto p = prev_nodes_.begin(); p != prev_nodes_.end(); p++) {
        ...
        Kmer prev_kmer = prev_node->kmer_;
...
size_t GraphAligner::add_sources(const Range &range, const Node &node) {
...
GraphAligner::Node *GraphAligner::add_child(Range &range, Node &node) {
...
std::vector<Result> GraphAligner::pop_seeds(std::ostream &out) { 
...
void GraphAligner::print_graph(bool verbose) {
