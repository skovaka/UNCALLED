...
int main(int argc, char **argv) {
    ...
    Normalizer win_norm(model, event_detection_defaults, 2000);
    ...
    while (getline(fast5_list, fast5_name)) {
        ...
        std::vector<Event> all_events = full_det.get_all_events(samples),
                           norm_events = all_events;

        full_det.reset();
        Normalizer full_norm(model, event_detection_defaults, all_events.size());
        model.normalize(norm_events);
        ...
        for (auto s : samples) {
            ...
            if (win_norm.add_sample(s)) {
                ...
                float e = win_norm.pop_event();
                std::cout << i << " "
                          << norm_events[i].mean - e << "\t"
                          << norm_events[i].mean << "\t" 
                          << e << "\n";
                i++;
            }
